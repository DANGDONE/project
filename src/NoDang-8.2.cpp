...
bool NoDang::insertPtrForValueOD(Value *uib, Value* modifiedPointer, Value* middlePointer, Value* oriP, Value* lastins, Function &F, bool ifGEPI)
{
    ...
    else if(StoreInst* SI = dyn_cast<StoreInst>(targetOper))
    {
        ...
        if(!ifGEPI)
        {
            //Simulate a case that p=q, the mod is newp=newq; p=q;
            bool ifstore = false;
            if((isa<AllocaInst>(rootDef) || isa<GlobalVariable>(rootDef)) && (rootDef == otherOper->user_back() || rootDef == otherOper))
            {
                StoreInst *newSI = nullptr;
                newSI = new StoreInst(SI->getValueOperand(), SI->getPointerOperand(), SI);
                ifstore = true;
                ifInsModified[newSI] = newSI;
            }
            if(!ifstore)
